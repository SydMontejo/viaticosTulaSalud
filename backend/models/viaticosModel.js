...
class ViaticosModel {
  static async crearViaticoCompleto(datos) {
    ...
    try {
      ...
      const numeroCaso = `CASO-${Date.now()
        .toString()
        .slice(-8)}-${Math.random().toString(36).substr(2, 4)}`;
  ...
  static async obtenerViaticoPorNumeroCaso(numeroCaso) {
    ...
    FROM viaticos v
    JOIN empleados e ON v.id_empleado = e.id_empleado
    JOIN detalles_viatico d ON v.id_viatico = d.id_viatico
    WHERE v.numero_caso = ?
  `,
      [numeroCaso]
    );
  ...
  static async actualizarEstado(numeroCaso, nuevoEstado, nombreJefe) {
    let fechaCambioEstado = null;
    if (nuevoEstado === "aprobado") {
      fechaCambioEstado = new Date(); // Fecha actual
    }
    ...
    if (fechaCambioEstado) {
      ...
      WHERE numero_caso = ?
    `;
      params = [nuevoEstado, nombreJefe, fechaCambioEstado, numeroCaso];
    } else {
      query = `
      UPDATE viaticos
      SET estado = ?, 
      ...
      WHERE numero_caso = ?
    `;
      params = [nuevoEstado, nombreJefe, numeroCaso];
    }
